    ...
    const html = `
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                ...
                <p class="text-2xl font-bold text-blue-600">${AppData.auditLogs.length}</p>
            </div>
            <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                <p class="text-sm text-gray-600">Today's Activity</p>
                <p class="text-2xl font-bold text-green-600">${AppData.auditLogs.filter(l => l.timestamp.includes(new Date().toLocaleDateString())).length}</p>
            </div>
            <div class="bg-purple-50 rounded-lg p-4 border border-purple-200">
                <p class="text-sm text-gray-600">Active Users</p>
                <p class="text-2xl font-bold text-purple-600">${[...new Set(AppData.auditLogs.map(l => l.user))].length}</p>
            </div>
        </div>
