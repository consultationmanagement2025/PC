...
print("=== REFACTORING REPORTS SECTION ONLY (DE-DUPLICATING CARDS & TABS) ===")
...
clean_reports_function = '''async function renderSystemReportsSection() {
    ...
    contentArea.innerHTML = `
        <div class="space-y-6">
            ...
            <div class="bg-gradient-to-r from-slate-900 via-red-950 to-slate-900 text-white p-7 rounded-2xl shadow-xl border border-red-950/40 flex flex-wrap items-center justify-between gap-4">
                <div>
                    <span class="px-3 py-1 rounded-full bg-white/15 text-red-100 text-[11px] font-extrabold uppercase tracking-wider backdrop-blur-xs border border-white/10">
                        <i class="bi bi-file-earmark-bar-graph-fill mr-1"></i> Policy Intelligence & Transmittal Hub
                    </span>
                ...
                <div class="flex items-center gap-2">
                    <button onclick="window.print()" class="px-4 py-2.5 bg-white text-slate-900 hover:bg-slate-100 text-xs font-bold rounded-xl transition shadow-sm flex items-center gap-1.5 cursor-pointer">
            ...
            <!-- Single Row Executive Metric Cards (Non-Redundant) -->
            ...
            <!-- Non-Redundant Focused Navigation Tabs (Audit & Document tabs removed) -->
    ...
    if os.path.exists(path):
        ...
        if 'async function renderSystemReportsSection()' in code:
            ...
            if start_fn != -1 and end_fn != -1:
                code = code[:start_fn] + clean_reports_function + '\n\n' + code[end_fn:]
                with open(path, 'w', encoding='utf-8') as f:
                    f.write(code)
                print(f"Updated renderSystemReportsSection in {path}")

print("Reports section refactor complete!")
