...
for filepath in files_to_update:
    ...
    # 1. Add "Forward to Expert" button into Admin Consultation Table Actions
    target_btn = '<button onclick="openExportChooser('
    replacement_btn = """<button onclick="openForwardToExpertModal(<?= (int)$c['id'] ?>, '<?= htmlspecialchars(addslashes($c['title'] ?? '')) ?>', '<?= htmlspecialchars(addslashes($c['category'] ?? '')) ?>')" class="text-red-700 hover:text-red-900 text-xs font-bold bg-red-50 hover:bg-red-100 px-2 py-0.5 rounded border border-red-200 inline-flex items-center gap-1"><i class="bi bi-send-fill text-[10px]"></i> Forward to Expert</button>\n                                                             <button onclick="openExportChooser("""

    if target_btn in code and 'openForwardToExpertModal' not in code:
    ...
    function openForwardToExpertModal(id, title, category) {
        ...
        // Populate Resource Person dropdown matching category
...
"""
    if 'id="forward-expert-modal"' not in code and '</body>' in code:
        code = code.replace('</body>', modal_html)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(code)

print("Forward to Expert button & modal added to all system-template-full.php files!")
