...
content = """<?php
/**
 * Resource Person Workspace - Clean Admin-Themed Dashboard with Inline Master Document Annotation Workflow
...
function ensureResourcePersonSchema($conn) {
    ...
    // 2. Consultations table additions for Inline Master Document
    $cCols = [];
...
if ($consultations_stmt) {
    ...
    $cRes = $conn->query("SELECT c.*, 0 as audit_count, 0 as info_requests_count FROM consultations c ORDER BY c.created_at DESC");
    if ($cRes) {
        while ($row = $cRes->fetch_assoc()) {
            $consultations[] = $row;
        }
...
foreach ($consultations as &$c) {
    ...
    if (in_array($status, ['completed', 'closed'])) {
        $completed_count++;
    } elseif (!$hasExpertNotes && ($status === 'pending' || $status === 'scheduled' || $isAssigned)) {
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2 text-xs text-slate-500">
                        ...
                        <span class="text-slate-800 font-medium">Single Master Consultation Document & Inline Advisory Workstation</span>
                    </div>
                </div>
            </div>
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <div class="bg-gradient-to-r from-red-700 via-red-800 to-red-900 text-white p-6 sm:p-8 rounded-2xl shadow-lg relative overflow-hidden">
                <div class="relative z-10 space-y-2">
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-red-100 text-xs font-medium backdrop-blur-sm">
                        <i class="bi bi-shield-check text-emerald-400"></i> Unified Master Document Collaboration Protocol
                    </div>
                    ...
                    <p class="text-[11px] text-red-200/90 pt-1">
                        <i class="bi bi-info-circle mr-1"></i> You are collaborating on <strong>ONE single consolidated master document</strong> per consultation. Annotations and technical recommendations are appended directly inline into the AI summary file forwarded by the Secretariat.
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Assigned Tasks</p>
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 mt-0.5">Single file updated</p>
                    </div>
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Completed / Closed</p>
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <h3 class="text-lg font-bold text-slate-900 flex items-center gap-2">
                        <i class="bi bi-file-earmark-word text-red-600"></i> Master Consultation Documents & Expert Advisory
                    </h3>
                            ...
                            <div class="task-card bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col justify-between p-6 space-y-4"
                                ...
                                <div class="space-y-3 pt-3 border-t border-slate-100">
                                    <div class="flex items-center justify-between text-xs">
                                        <span class="text-[11px] text-slate-500 font-medium"><i class="bi bi-calendar3 mr-1"></i> Due <?php echo $c['deadline_formatted']; ?></span>
        ...
        <div class="bg-white rounded-3xl shadow-2xl max-w-3xl w-full p-6 sm:p-8 relative border border-slate-200 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                    <h4 class="font-bold text-slate-900 flex items-center gap-1.5"><i class="bi bi-file-earmark-text text-red-600"></i> Master Document Protocol</h4>
                    <p class="text-slate-600">Each consultation contains a single master document. Resource Persons append their annotations directly inline to preserve a single consolidated file for committees.</p>
                </div>
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
...
print("Successfully written single master document inline annotation dashboard!")
