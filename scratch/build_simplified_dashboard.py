...
if ($consultations_stmt) {
    ...
    $cRes = $conn->query("SELECT c.*, 0 as reports_count, 0 as approved_reports_count, 0 as info_requests_count FROM consultations c ORDER BY c.created_at DESC");
    if ($cRes) {
        while ($row = $cRes->fetch_assoc()) {
            $consultations[] = $row;
        }
...
foreach ($consultations as &$c) {
    ...
    if (in_array($status, ['completed', 'closed', 'endorsed'])) {
        $completed_count++;
    } elseif ($reportsCount == 0 && ($status === 'pending' || $status === 'scheduled' || $isAssigned)) {
...
<body class="bg-slate-100 text-slate-800 antialiased min-h-screen flex flex-col md:flex-row">
    ...
    <!-- Admin-Style Collapsible Sidebar (Matches Admin Dashboard Theme) -->
        ...
        <nav class="flex-1 py-4 px-3 overflow-y-auto space-y-6">
            <div>
                ...
                <div class="space-y-1">
                    ...
                    <a href="#reports" onclick="filterTaskCategory('reports')" class="flex items-center px-4 py-3 text-red-100 hover:bg-red-700/60 hover:text-white rounded-xl text-sm transition gap-3">
    ...
    <div class="flex-1 flex flex-col min-w-0 min-h-screen">
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <!-- Clean KPI Metric Summary Cards (Matches Admin Dashboard Grid Style) -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 mt-0.5">Assigned by Secretariat</p>
                    </div>
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pending Reviews</p>
                        <p class="text-3xl font-bold text-amber-600 mt-1"><?php echo $pending_reviews_count; ?></p>
                        <p class="text-[11px] text-slate-400 mt-0.5">Awaiting resolution report</p>
                    </div>
            ...
            <!-- Toolbar Filters Section (Admin Style) -->
                    ...
                    <?php else: ?>
                        <?php foreach ($consultations as $c): ?>
                            <?php 
                                ...
                                $deadlineStatus = $c['deadline_status'] ?? 'normal';
                                $deadlineText = $c['deadline_formatted'] ?? '';
                                $deadlineBadge = '<span class="text-[11px] text-slate-500 font-medium"><i class="bi bi-calendar3 mr-1"></i> Due ' . $deadlineText . '</span>';
                                if ($deadlineStatus === 'overdue') {
                                    $deadlineBadge = '<span class="text-[11px] text-rose-700 font-bold"><i class="bi bi-exclamation-triangle-fill mr-1"></i> Overdue (' . $deadlineText . ')</span>';
                                } elseif ($deadlineStatus === 'due_soon') {
                                    $deadlineBadge = '<span class="text-[11px] text-amber-700 font-bold"><i class="bi bi-clock-fill mr-1"></i> Due Soon (' . $deadlineText . ')</span>';
                                }

                                $cardTag = $isAssignedToMe ? 'assigned' : ($isExpertiseMatch ? 'expertise' : 'general');
                                if ($deadlineStatus === 'overdue' || $deadlineStatus === 'due_soon') $cardTag .= ' overdue';
                                $reportsCount = (int)($c['reports_count'] ?? 0);
                            ...
                            <div class="task-card bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col justify-between p-6 space-y-4"
                                ...
                                <div class="space-y-3 pt-3 border-t border-slate-100">
                                    ...
                                    <div class="flex items-center justify-between text-xs text-slate-500">
                                        <span class="flex items-center gap-1 font-medium"><i class="bi bi-file-earmark-check text-emerald-600"></i> <?php echo $reportsCount; ?> Reports</span>
                                        <span class="flex items-center gap-1 font-medium"><i class="bi bi-question-circle text-amber-600"></i> <?php echo $infoRequestsCount; ?> Info Req</span>
                                    </div>

                                    <div class="grid grid-cols-3 gap-2">
                                        <button onclick="openUploadModal(<?php echo $c['id']; ?>, '<?php echo htmlspecialchars(addslashes($c['title'])); ?>')" 
                                                class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-2 rounded-xl text-xs transition flex items-center justify-center gap-1 shadow-sm">
                                        ...
                                        <button onclick="openDetailsModal(<?php echo $c['id']; ?>, '<?php echo htmlspecialchars(addslashes($c['title'])); ?>')" 
    ...
    <!-- Upload Resolution Report Modal -->
        ...
        <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-6 sm:p-8 relative border border-slate-200 space-y-5">
            ...
            <form action="API/upload_resolution_report.php" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="version_label" placeholder="e.g. v1.0, v1.1, v2.0 Final" 
    ...
    <!-- Details & Version History Modal -->
    <div id="details-modal" class="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-6 sm:p-8 relative border border-slate-200 space-y-5">
            <button onclick="closeDetailsModal()" class="absolute top-5 right-5 text-slate-400 hover:text-slate-600 text-lg">
                <i class="bi bi-x-lg"></i>
            </button>

            <div class="border-b border-slate-100 pb-3">
                <h3 class="text-lg font-bold text-slate-900" id="details-modal-title">Consultation Details</h3>
                <p class="text-xs text-slate-500" id="details-modal-category">Version History & Report Log</p>
            </div>

            <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                <div>
                    <h4 class="font-bold text-xs text-slate-800 uppercase tracking-wider mb-2">Uploaded Resolution Papers</h4>
                    <div class="border border-slate-200 rounded-xl overflow-hidden">
                        <table class="w-full text-left text-xs border-collapse">
                            ...
                            <tbody id="details-modal-reports-body" class="divide-y divide-slate-100 bg-white">
                                <tr><td colspan="4" class="p-4 text-center text-slate-400">Loading version history...</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <div class="flex justify-end pt-2">
                <button type="button" onclick="closeDetailsModal()" class="px-5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl text-xs transition">
                    Close
                </button>
            </div>
        </div>
    ...
    <div id="knowledge-base-modal" class="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl shadow-2xl max-w-3xl w-full p-6 sm:p-8 relative border border-slate-200 space-y-5">
            <button onclick="closeKnowledgeBaseModal()" class="absolute top-5 right-5 text-slate-400 hover:text-slate-600 text-lg">
                <i class="bi bi-x-lg"></i>
            </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                    <h4 class="font-bold text-slate-900 flex items-center gap-1.5"><i class="bi bi-file-earmark-text text-red-600"></i> Resolution Report Guidelines</h4>
                    <p class="text-slate-600">Reports should include executive summary, legal/technical rationale, public sentiment findings, and explicit policy recommendations for LGU committees.</p>
                </div>
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                    <h4 class="font-bold text-slate-900 flex items-center gap-1.5"><i class="bi bi-shield-check text-emerald-600"></i> Data Privacy Compliance</h4>
                    <p class="text-slate-600">In accordance with RA 10173, citizen personally identifiable information (PII) must be anonymized in all published resolution papers.</p>
                </div>
            </div>

            <div class="flex justify-end pt-2">
                <button type="button" onclick="closeKnowledgeBaseModal()" class="px-5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl text-xs transition">
                    Close
                </button>
            </div>
        </div>
    ...
    function openDetailsModal(id, title) {
        document.getElementById('details-modal-title').textContent = title;
        const tbody = document.getElementById('details-modal-reports-body');
        tbody.innerHTML = '<tr><td colspan="4" class="p-4 text-center text-slate-400">Loading version history...</td></tr>';
        ...
        .then(data => {
            if (data.success && data.reports && data.reports.length > 0) {
                tbody.innerHTML = data.reports.map(r => `
                    <tr>
                        <td class="p-2.5 font-bold text-red-800">${r.version_label || 'v1.0'}</td>
                        <td class="p-2.5 text-slate-500">${r.created_at || 'N/A'}</td>
                        <td class="p-2.5"><span class="px-2 py-0.5 rounded text-[9px] font-bold uppercase bg-emerald-100 text-emerald-800">${r.status || 'Submitted'}</span></td>
                        <td class="p-2.5 text-center">
                            <a href="uploads/resolution_reports/${r.file_path}" target="_blank" download class="text-xs text-blue-600 hover:underline font-bold">Download</a>
                        </td>
                    </tr>
                `).join('');
            } else {
                tbody.innerHTML = '<tr><td colspan="4" class="p-4 text-center text-slate-400">No reports uploaded yet.</td></tr>';
            }
        });
...
with open(r'c:\xampp\htdocs\CAP101\PC\resource_person_dashboard.php', 'w', encoding='utf-8') as f:
    f.write(simplified_php)

print("Successfully written clean admin-themed resource_person_dashboard.php!")
