...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("SELECT id, fullname, password, role FROM users WHERE LOWER(TRIM(email))=LOWER(TRIM(?)) OR LOWER(TRIM(username))=LOWER(TRIM(?))");
    if (!$stmt) {
    ...
    if (!$stmt) {
        ...
        if ($user = $result->fetch_assoc()) {
            ...
            if ($passValid) {
                ...
                $redirectUrl = ($isAdmin) ? "system-template-full.php" : "index.php";
...
<body class="bg-gradient-to-br from-red-50 via-white to-red-50 min-h-screen flex items-center justify-center p-3 md:p-4">
    ...
    <script>
        // Final password toggle setup at end of page
