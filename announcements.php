...
function createAnnouncement($admin_id, $admin_user, $title, $content, $visibility = 'public') {
    ...
    $stmt = $conn->prepare("INSERT INTO announcements (title, content, admin_id, admin_user, visibility) VALUES (?, ?, ?, ?, ?)");
    if (!$stmt) return false;
    $stmt->bind_param('ssiss', $title, $content, $admin_id, $admin_user, $visibility);
    $res = $stmt->execute();
    $stmt->close();
