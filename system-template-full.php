...
<body class="bg-gray-100 font-sans antialiased">
                        ...
                        <div class="bg-white rounded-lg shadow-md p-6">
                            <div class="flex justify-between items-center mb-6">
                                <div>
                                    ...
                                    <p class="text-gray-600 text-sm mt-1">Manage citizen submitters and staff accounts</p>
                                </div>
                            </div>
    ...
    <style>
        #admin-chatbot-btn { position:fixed; bottom:24px; right:24px; z-index:9998; width:56px; height:56px; border-radius:50%; background:linear-gradient(135deg,#991b1b,#7f1d1d); color:#fff; border:none; cursor:pointer; box-shadow:0 6px 24px rgba(153,27,27,0.4); display:flex; align-items:center; justify-content:center; font-size:1.5rem; transition:all 0.3s; }
        #admin-chatbot-btn:hover { transform:scale(1.1); box-shadow:0 8px 30px rgba(153,27,27,0.5); }
        #admin-chatbot-btn .badge-dot { position:absolute; top:2px; right:2px; width:12px; height:12px; background:#10b981; border-radius:50%; border:2px solid #fff; }
        #admin-chat-window { position:fixed; bottom:90px; right:24px; z-index:9999; width:380px; max-height:520px; background:#fff; border-radius:16px; box-shadow:0 12px 40px rgba(0,0,0,0.2); display:none; flex-direction:column; overflow:hidden; }
        #admin-chat-window.open { display:flex; }
        #admin-chat-header { background:linear-gradient(135deg,#991b1b,#7f1d1d); color:#fff; padding:16px; display:flex; align-items:center; gap:12px; }
        #admin-chat-header .avatar { width:40px; height:40px; border-radius:50%; background:rgba(255,255,255,0.2); display:flex; align-items:center; justify-content:center; font-size:1.25rem; }
        #admin-chat-messages { flex:1; overflow-y:auto; padding:16px; display:flex; flex-direction:column; gap:12px; max-height:340px; min-height:200px; background:#f9fafb; }
        .admin-msg { max-width:85%; padding:10px 14px; border-radius:12px; font-size:0.875rem; line-height:1.5; word-wrap:break-word; }
        .admin-msg.bot { background:#fff; border:1px solid #e5e7eb; color:#374151; align-self:flex-start; border-bottom-left-radius:4px; }
        .admin-msg.user { background:linear-gradient(135deg,#991b1b,#b91c1c); color:#fff; align-self:flex-end; border-bottom-right-radius:4px; }
        .admin-msg.bot strong { color:#991b1b; }
        #admin-chat-input-area { padding:12px; border-top:1px solid #e5e7eb; display:flex; gap:8px; align-items:flex-end; background:#fff; }
        #admin-chat-input { flex:1; border:1.5px solid #e5e7eb; border-radius:12px; padding:10px 14px; font-size:0.875rem; outline:none; resize:none; font-family:inherit; line-height:1.4; max-height:90px; overflow-y:auto; }
        #admin-chat-input:focus { border-color:#991b1b; box-shadow:0 0 0 3px rgba(153,27,27,0.1); }
        #admin-chat-send { width:40px; height:40px; border-radius:10px; background:#991b1b; color:#fff; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; margin-bottom:2px; }
        #admin-chat-send:hover { background:#7f1d1d; }
        .admin-typing { display:flex; gap:4px; padding:10px 14px; align-self:flex-start; }
        .admin-typing span { width:8px; height:8px; background:#9ca3af; border-radius:50%; animation:adminBounce 1.4s infinite; }
        .admin-typing span:nth-child(2) { animation-delay:0.2s; }
        .admin-typing span:nth-child(3) { animation-delay:0.4s; }
        @keyframes adminBounce { 0%,80%,100%{transform:translateY(0)} 40%{transform:translateY(-6px)} }

        /* Dark mode */
        .dark #admin-chat-window { background:#1f2937 !important; }
        .dark #admin-chat-messages { background:#111827 !important; }
        .dark .admin-msg.bot { background:#374151 !important; border-color:#4b5563 !important; color:#e5e7eb !important; }
        .dark .admin-msg.bot strong { color:#fca5a5 !important; }
        .dark #admin-chat-input-area { background:#1f2937 !important; border-color:#374151 !important; }
        .dark #admin-chat-input { background:#374151 !important; border-color:#4b5563 !important; color:#e5e7eb !important; }
    </style>

    <button id="admin-chatbot-btn" onclick="toggleAdminChat()" title="AI Assistant">
    ...
    <div id="admin-chat-window">
        <div id="admin-chat-header">
            ...
            <div style="flex:1;">
                <div style="font-weight:800; font-size:0.95rem;">Admin AI Assistant</div>
                <div style="font-size:0.7rem; opacity:0.8;">System Navigation & Help</div>
            </div>
            <button onclick="toggleAdminChat()" style="background:none; border:none; color:#fff; font-size:1.25rem; cursor:pointer; opacity:0.8;">&times;</button>
        </div>
        <div id="admin-chat-messages">
            <div class="admin-msg bot">Hello! 👋 I'm your <strong>Admin AI Assistant</strong>. I can help you navigate the system, manage consultations, feedback, users, and more.<br><br>What would you like help with?</div>
        </div>
        <div id="admin-chat-input-area">
            <textarea id="admin-chat-input" placeholder="Ask me anything..." rows="2" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();sendAdminChat();}"></textarea>
            <button id="admin-chat-send" onclick="sendAdminChat()"><i class="bi bi-send"></i></button>
        </div>
    </div>
    ...
    function sendAdminChat() {
        ...
        if (!msg) return;
        input.value = '';
        appendAdminMsg(msg, 'user');
        showAdminTyping();

        fetch('API/admin_chatbot_api.php', {
            method: 'POST',
        ...
        .then(function(data) {
            ...
            if (data.success && data.reply) {
                appendAdminMsg(data.reply, 'bot');
            } else {
                appendAdminMsg('Sorry, I encountered an error. Please try again.', 'bot');
            }
        })
        .catch(function() {
            removeAdminTyping();
            appendAdminMsg('Sorry, I\'m unable to connect right now. Please try again.', 'bot');
        });
    }
    </script>
</body>
