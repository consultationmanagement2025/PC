...
if (file_exists('db.php')) {
    ...
    $row = $conn->query("SELECT
            ...
            FROM consultations")->fetch_assoc();
    if ($row) {
    ...
    // Status breakdown
    $res = $conn->query("SELECT status, COUNT(*) AS total FROM consultations GROUP BY status ORDER BY total DESC");
    if ($res) {
    ...
    // Top categories
    $res = $conn->query("SELECT COALESCE(category,'Uncategorized') AS category, COUNT(*) AS total FROM consultations GROUP BY category ORDER BY total DESC LIMIT 10");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
    ...
    $res = $conn->query("SELECT id, title, category, status, type, created_at FROM consultations ORDER BY created_at DESC LIMIT 6");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
...
function generateModuleReportFile($module, $format, $conn, $user_name = null) {
    ...
    if ($format === 'excel') {
        ...
        $page_stream = "BT\n/F1 12 Tf\n50 770 Td\n(" . str_replace(["\\", "(", ")"], ["\\\\", "\\(", "\\)"], $report_data['title']) . ") Tj\n";
        $y = 740;
        ...
        $write_success = file_put_contents($file_path, $pdf_content) !== false;
        error_log('PDF write success: ' . ($write_success ? 'yes' : 'no'));
    }
...
<body class="bg-gray-100 font-sans antialiased">
    ...
    <div id="mobile-sidebar" class="fixed inset-y-0 left-0 transform -translate-x-full md:hidden w-72 bg-gradient-to-b from-red-800 to-red-900 text-white z-50 transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] overflow-hidden flex flex-col shadow-2xl">
        ...
        <nav class="flex-1 py-4 px-3 overflow-y-auto">
            ...
            <?php if ($is_admin_or_super || $is_barangay_staff): ?>
        ...
        <aside id="sidebar" class="sidebar sidebar-expanded w-64 bg-gradient-to-b from-red-800 to-red-900 text-white flex-shrink-0 flex flex-col transition-all duration-300 ease-in-out animate-slide-in-left h-screen fixed md:relative z-30 -translate-x-full md:translate-x-0">
            ...
            <nav class="flex-1 overflow-y-auto py-4">
                ...
                <div class="px-4 space-y-1">
                    ...
                    <!-- User Management hidden for presentation -->
                    ...
                    <section id="user-management-section" class="mb-6" style="display: none;">
                        ...
                        <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            <div class="flex justify-between items-center mb-6">
                                ...
                                <button type="button" onclick="openModuleReportModal('users')" class="btn-outline px-4 py-2">
                                    <i class="bi bi-file-earmark-bar-graph mr-2"></i>Generate Report
                                </button>
    ...
    <script>
        ...
        window.__IS_BARANGAY_STAFF__ = <?php echo $is_barangay_staff ? 'true' : 'false'; ?>;
