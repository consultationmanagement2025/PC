...
function logAction($admin_id, $admin_user, $action, $entity_type = null, $entity_id = null, $old_value = null, $new_value = null, $status = 'success', $details = null) {
    ...
    if ($result && shouldSendEmailNotification($action)) {
        sendAdminEmailNotification($admin_user, $action, $entity_type, $status);
    }
...
// ========================================
// Format Audit Log Action Summary
// ========================================
function getAuditActionSummary($log) {
...
// ========================================
function shouldSendEmailNotification($action) {
    $critical_actions = [
        'Created user',
        'Deleted user', 
        'Created consultation',
    ...
    foreach ($critical_actions as $critical) {
        if (stripos($action, $critical) !== false) {
            return true;
        }
    }
    return false;
...
function sendAdminEmailNotification($admin_user, $action, $entity_type, $status) {
    $subject = "Admin Action Alert - Valenzuela City System";
    $body = "An important admin action was performed in the Valenzuela City Consultation System:\n\n";
    $body .= "Admin: " . $admin_user . "\n";
    $body .= "Action: " . $action . "\n";
    $body .= "Entity: " . ($entity_type ?? 'N/A') . "\n";
    $body .= "Status: " . $status . "\n";
    $body .= "Time: " . date('Y-m-d H:i:s') . "\n";
    $body .= "IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'N/A') . "\n\n";
    $body .= "Please review this action if necessary.";
    
    // Send to the admin Gmail account
    sendGmailEmail('consultationmanagement2025@gmail.com', $subject, $body, false);
}
