...
try {
    switch ($action) {
        case 'list_pending':
            ...
            $stmt = $conn->prepare("SELECT id, fullname, email, expertise_areas, qualifications, department, phone, created_at FROM users WHERE role = 'resource person' AND approval_status = 'pending' ORDER BY created_at DESC");
            $stmt->execute();
        ...
        case 'approve':
            ...
            $stmt = $conn->prepare("UPDATE users SET approval_status = 'approved', approved_by = ?, approved_at = NOW() WHERE id = ? AND role = 'resource person' AND approval_status = 'pending'");
        ...
        case 'reject':
            ...
            $stmt = $conn->prepare("UPDATE users SET approval_status = 'rejected', approved_by = ?, approved_at = NOW() WHERE id = ? AND role = 'resource person' AND approval_status = 'pending'");
            $stmt->bind_param('ii', $admin_id, $user_id);
        ...
        case 'list_approved':
            ...
            $stmt = $conn->prepare("SELECT id, fullname, email, expertise_areas, qualifications, department, phone, approved_at FROM users WHERE role = 'resource person' AND approval_status = 'approved' ORDER BY approved_at DESC");
            $stmt->execute();
            $result = $stmt->get_result();
            $resource_persons = [];
            while ($row = $result->fetch_assoc()) {
