    ...
    switch ($action) {
        case 'list':
            ...
            $all_docs = array_map(function ($row) {
                ...
                if ($row['source'] === 'consultation' && in_array($uploaderRole, ['admin', 'administrator', 'super admin', 'superadmin', 'staff', 'resource person', 'resource_person'], true)) {
                    $row['source'] = 'admin';
                }
                return $row;
            }, $all_docs);
        ...
        case 'update_status':
            $id = (int)($data['id'] ?? $data['document_id'] ?? ($_POST['id'] ?? 0));
            $newStatus = trim((string)($data['status'] ?? ($_POST['status'] ?? '')));
            if ($id <= 0 || empty($newStatus)) {
                http_response_code(400);
                echo json_encode(['success' => false, 'message' => 'Missing ID or status']);
                exit;
            }
            if (function_exists('updateDocumentStatus')) {
                $ok = updateDocumentStatus($id, $newStatus);
                echo json_encode(['success' => (bool)$ok]);
            } else {
