...
function renderAnalyticsCharts(data) {
    ...
    if (postsChartCtx) {
        ...
        new Chart(postsChartCtx, {
            type: 'line',
            data: {
                labels: dailyPosts.map(d => new Date(d.date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })),
                datasets: [{
                    label: 'Posts',
    ...
    if (usersChartCtx) {
        ...
        new Chart(usersChartCtx, {
            type: 'bar',
            data: {
                labels: dailyUsers.map(d => new Date(d.date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })),
                datasets: [{
                    label: 'New Users',
    ...
    fetch('API/users_api.php?action=list')
        ...
        .then(result => {
            ...
            tbody.innerHTML = users.map(user => {
                ...
                const lastLogin = user.last_login ? new Date(user.last_login).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }) : 'Never';
...
function saveUser() {
    ...
    if (id) {
        ...
        if (user) {
            ...
            user.status = status;
            if (lastLogin) user.lastLogin = new Date(lastLogin).toLocaleString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
        ...
        const newUser = {
            ...
            department,
            status,
            lastLogin: new Date().toLocaleString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })
        };
...
function displayAdminAnnouncements() {
    ...
    list.innerHTML = AppData.announcements.map(a => `
        <div class="p-2.5 border border-gray-200 rounded hover:bg-gray-50 transition text-xs">
            ...
            <div class="text-gray-500 text-xs mt-0.5">${new Date(a.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}</div>
...
function renderConsultationsTableFromData(consultations) {
    ...
    tbody.innerHTML = consultations.map(consultation => {
        ...
        const startDate = consultation.start_date ? new Date(consultation.start_date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-';
        const endDate = consultation.end_date ? new Date(consultation.end_date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-';
    ...
    tbody.innerHTML = consultations.map(consultation => {
        ...
        return `
            <tr class="border-b hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4 text-gray-600">${new Date(consultation.date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}</td>
                <td class="px-6 py-4">
                    <span class="px-3 py-1 rounded-full text-xs font-semibold ${statusColor}">
                        ${consultation.status}
                    </span>
    ...
    tbody.innerHTML = feedbackList.map(feedback => {
        ...
        return `
            <tr class="border-b hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4 text-gray-600">${new Date(feedback.date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}</td>
                <td class="px-6 py-4 text-center">
                    <div class="flex gap-2 justify-center">
