...
if (!function_exists('sendToOrtsApi')) {
    function sendToOrtsApi($consultationId, $conn) {
        ...
        $stmt = $conn->prepare("SELECT id, title, description, ai_committee_brief, status, committee_assigned FROM consultations WHERE id = ? LIMIT 1");
        if (!$stmt) {
        ...
        $refNo = "CONSULT-" . sprintf("%06d", $consultationId);
        ...
        // Update consultation status to forwarded_orts if successful
        if (!empty($res['success'])) {
            $upStmt = $conn->prepare("UPDATE consultations SET status = 'forwarded_orts', committee_forwarded_at = NOW() WHERE id = ?");
            if ($upStmt) {
