...
/**
 * Detect inappropriate or abusive content in text
 */
function containsInappropriateContent($text) {
    ...
    $badWords = [
        'fuck', 'shit', 'bitch', 'asshole', 'pussy', 'dick', 'penis', 'vagina', 'nazi',
        'terrorist', 'bomb', 'drugs', 'drug', 'kill', 'murder', 'rape', 'porn', 'viagra',
        'scam', 'fraud', 'fake', 'hate', 'slur'
    ];
...
/**
 * Detect likely spam submissions in feedback content
 */
function isSpamFeedback($name, $email, $message) {
    ...
    if (preg_match('/\b(click here|buy now|free money|loan offer|work from home|cheap pills|bit\.ly|tinyurl|sex|escort)\b/i', $message)) {
        return true;
    }
    if (!empty($email) && preg_match('/\b(no-reply|noreply|spam|disposable|temporary)\b/i', $email)) {
        return true;
    }
    return false;
}
...
/**
 * Require resource person role and valid session
 */
function requireResourcePersonRole() {
    if (!checkSessionTimeout()) {
        header('Location: login.php?error=Session%20expired');
        exit;
    }
    ...
    // Check if resource person is approved
    if (isset($_SESSION['user_id'])) {
        global $conn;
        $stmt = $conn->prepare("SELECT approval_status FROM users WHERE id = ?");
...
/**
 * Check if user is admin or staff (including resource person)
 */
function requireAdminOrStaffRole() {
    if (!checkSessionTimeout()) {
        header('Location: login.php?error=Session%20expired');
        exit;
    }
